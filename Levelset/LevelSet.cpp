...
glm::vec3 LevelSet::GetGradient(float x, float y, float z) const {
    ...
    float diffxpm, diffypm, diffzpm;
    diffxpm = DiffXpm(i, j, k);
    diffypm = DiffYpm(i, j, k);
    diffzpm = DiffZpm(i, j, k);

    glm::vec3 gradient(diffxpm, diffypm, diffzpm);
...
float LevelSet::DiffZm(size_t i, size_t j, size_t k) const {
    return (mGrid.GetValue(i, j, k + 1) - mGrid.GetValue(i, j, k - 1)) / mDx;
}
